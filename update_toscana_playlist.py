...
JSON_DB   = 'radio_toscana_history.json'
EXCEL_OUT = 'classifica_radio_toscana_STORICO.xlsx'
URL_BASE  = "https://onlineradiobox.com/it/toscana/playlist/"
...
def scrape_day(offset=0):
    ...
    try:
        ...
        print(f"Errore scraping Toscana offset {offset}: {e}", flush=True)
        return []
...
def main():
    ...
    print(f"Aggiunti {added_count} nuovi passaggi Toscana.", flush=True)
    ...
    ws = workbook.add_worksheet('Classifica Toscana')

    header_fmt = workbook.add_format({'bold': True, 'bg_color': '#E2EFDA', 'border': 1, 'align': 'center'})
    ...
    ws.freeze_panes(1, 4)
    workbook.close()
    print(f"Excel Toscana aggiornato: {EXCEL_OUT}", flush=True)
