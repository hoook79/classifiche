...
// Trigger per gestire la multiscelta (menu a tendina con spunte) nella colonna AllowedRadios della scheda Users
function onEdit(e) {
  var sheet = e.source.getActiveSheet();
  var range = e.range;
  
  // Controlla se l'evento è sulla scheda "Users", nella colonna 5 (E) e non è l'intestazione
  if (sheet.getName() === "Users" && range.getColumn() === 5 && range.getRow() > 1) {
    var oldValue = e.oldValue;
    var newValue = e.value;
    
    // Se la cella è stata svuotata, non fare nulla
    if (!newValue) {
      return;
    }
    
    // Se prima era vuota o è impostata a "all" o "*", o se il nuovo valore è "all" o "*"
    if (!oldValue || oldValue === "all" || oldValue === "*" || newValue === "all" || newValue === "*") {
      return;
    }
    ...
    // Suddivide il vecchio valore in un array
    var list = oldValue.split(",").map(function(item) {
      return item.trim();
    ...
    var index = -1;
    // Cerca il nuovo valore nell'array ignorando maiuscole/minuscole
    for (var i = 0; i < list.length; i++) {
      if (list[i].toLowerCase() === newValue.toLowerCase()) {
        index = i;
        break;
      }
    }
    ...
    if (index > -1) {
      // Se era già presente, lo rimuoviamo (togliamo la spunta)
      list.splice(index, 1);
    } else {
      // Se non era presente, lo aggiungiamo (mettiamo la spunta)
      list.push(newValue);
    }
    
    // Costruisce la nuova stringa ordinata e formattata
    if (list.length === 0) {
      range.setValue("");
    } else {
